...
| **Chunked O(N) SSM/RWKV forward** | §7 #4 | Currently materialized O(n²) (`ssm_decayed_numerator` builds the full [seq,seq] decay-masked score matrix). Pure throughput — same function. **Tractable + safe to verify**: equivalence to the existing `ssm()` / `cpu_ssm` reference + a finite-diff grad-check (no arch/checkpoint change). Chunk by C: intra-chunk = materialized C×C attention; inter-chunk = decayed running state S passed forward (`trans_a` state primitive, same as linear attention). The decay bookkeeping is the only subtlety — verify against `ssm()` element-wise before trusting. |
