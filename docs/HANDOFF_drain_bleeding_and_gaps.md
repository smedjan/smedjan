...
| **Chunked O(N) SSM forward** | §7 #4 | ✅ **LANDED** — `ssm_decayed_numerator_chunked` (Mamba-2/SSD), wired into `ssm()` for seq≥256 (largest of {64,32} dividing seq). Intra-chunk = the materialised primitive per chunk; inter-chunk = `o=g·(q@S_p)` with state `S=Λ@KV` — a chunk-level decay matrix applied as ONE batched `[nc,nc]@[nc,hd²]`, **no sequential scan**. Decay factorisation `g_t·Λ(p',p)·h_j = exp(A_t−A_j)` verified algebraically + `ssm_chunked_matches_materialized` (forward, every chunk size) + `ssm_chunked_grad_matches_materialized` (backward q/k/v/loga). RWKV chunking still open (wkv uses a per-channel, not per-position, decay). **Non-bug noted:** loga position 0 has a STRUCTURALLY-ZERO gradient (uniform shift of A, cancels from every `A_t−A_j`) — a finite-diff there is fp-noise×1/2ε and spuriously "fails"; verify SSM loga grads by analytic-equivalence, not central differences. |
