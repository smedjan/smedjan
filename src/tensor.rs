...
impl Tensor {
    ...
    /// Precise (full-FP32) 2D matmul: C = self @ other with no fp16 truncation or ±65504 clamp on
    /// the operands. ~2× slower than `matmul` (fp32 shared tiles) — opt-in for precision/range
    /// sensitive matmuls (e.g. logits, large-magnitude values). Records Op::Matmul, so the backward
    /// uses the standard path; this is a forward-precision tool.
    pub fn matmul_precise(&self, other: &Tensor) -> Tensor {
        assert_eq!(self.shape.len(), 2, "matmul_precise expects 2D tensors");
    ...
    /// Block-mean keys for block-sparse attention. `self` is K `[bh, seq, hd]`; returns
    /// `[bh, nb, hd]` where each block of `block_size` positions is averaged. NOT recorded on the
    /// tape — these means only drive the (non-differentiable) top-k block selection, like hard
    /// attention; gradients flow through the kept dense scores, not the routing.
    pub fn block_mean_keys(&self, block_size: usize) -> Tensor {
        assert_eq!(self.shape.len(), 3, "block_mean_keys expects [bh, seq, hd]");
    ...
    /// Top-k block-sparse attention mask (MoBA/NSA routing). `self` is dense scores `[bh, seq, seq]`;
    /// `block_scores` is `[bh, seq, nb]` (query · block-mean-key). Masks every position outside the
    ...
    pub fn broadcast_rows(&self, rows: usize) -> Tensor {
        let cols = self.numel();
        let out_buf = self.ctx.alloc_buffer(rows * cols * 4);
        compute::gpu_broadcast_rows(&self.ctx, &self.buffer, &out_buf, rows as u32, cols as u32);
