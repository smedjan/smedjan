...
pub fn default_templates() -> Vec<PromptTemplate> {
    vec![
        PromptTemplate {
            ...
            system: "You are a clear, logical thinker. Explain your reasoning step by step.".to_string(),
            prompts: vec![
                ...
                "Explain the difference between correlation and causation with examples.".to_string(),
                "What are the trade-offs between microservices and monolithic architecture?".to_string(),
            ],
        },
        PromptTemplate {
        ...
        PromptTemplate {
            ...
            prompts: vec![
                "What are the pros and cons of remote work for companies and employees?".to_string(),
...
pub fn generate_training_data(config: &DistillConfig) -> std::io::Result<usize> {
    ...
    eprintln!("Generating training data from {} ({})", config.model, config.api_url);
    eprintln!("Templates: {} categories, {} prompts each", templates.len(), templates[0].prompts.len());

    for template in &templates {
        for prompt in &template.prompts {
            if total_pairs >= config.n_samples { break; }

            // Build API request based on URL pattern
            let response = if config.api_url.contains("anthropic.com") {
                call_claude_api(&config.api_url, &config.api_key, &config.model,
                    &template.system, prompt, config.max_tokens, config.temperature)
            } else if config.api_url.contains("openai.com") {
                call_openai_api(&config.api_url, &config.api_key, &config.model,
                    &template.system, prompt, config.max_tokens, config.temperature)
            } else {
                // Ollama/local API
                call_ollama_api(&config.api_url, &config.model,
            ...
            match response {
                Ok(text) => {
                    ...
                    writeln!(output, "{{\"prompt\":\"{}\",\"response\":\"{}\",\"category\":\"{}\"}}",
                ...
                Err(e) => {
                    eprintln!("  API error for '{}': {}", &prompt[..prompt.len().min(40)], e);
                }
            }
        }
    }

    eprintln!("Generated {} training pairs → {}", total_pairs, config.output_path);
    Ok(total_pairs)
}

/// Call Claude API (Anthropic Messages API).
fn call_claude_api(url: &str, api_key: &str, model: &str, system: &str, prompt: &str, max_tokens: usize, temperature: f32) -> Result<String, String> {
    ...
    Err(format!("Failed to parse Claude response: {}", &response[..response.len().min(200)]))
}

/// Call OpenAI-compatible API.
fn call_openai_api(url: &str, api_key: &str, model: &str, system: &str, prompt: &str, max_tokens: usize, temperature: f32) -> Result<String, String> {
    let body = format!(
    ...
    Err(format!("Failed to parse OpenAI response: {}", &response[..response.len().min(200)]))
}

/// Call Ollama local API.
fn call_ollama_api(url: &str, model: &str, prompt: &str, max_tokens: usize, temperature: f32) -> Result<String, String> {
    let body = format!(
