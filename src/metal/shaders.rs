...
/// Each threadgroup handles one token. d_model ≤ 256, rank ≤ 64.
pub const FUSED_PRE_ATTN: &str = r#"
#include <metal_stdlib>
using namespace metal;
struct PreAttnParams {
    uint batch_tokens; uint d_model; uint n_heads; uint n_kv_heads;
    uint head_dim; uint seq_len; uint rank; float eps; float rope_theta; uint kv_offset;
};
kernel void fused_pre_attn(
    device const float* x [[buffer(0)]], device const float* norm_w [[buffer(1)]],
    device const float* W_q [[buffer(2)]], device const float* W_k [[buffer(3)]],
    device const float* W_v [[buffer(4)]], device const float* W_q_v [[buffer(5)]],
    device const float* W_k_v [[buffer(6)]], device const float* W_v_v [[buffer(7)]],
    device float* Q_out [[buffer(8)]], device float* K_out [[buffer(9)]],
    device float* V_out [[buffer(10)]], constant PreAttnParams& p [[buffer(11)]],
    uint gid [[threadgroup_position_in_grid]], uint tid [[thread_index_in_threadgroup]],
    uint tpg [[threads_per_threadgroup]], uint sl [[thread_index_in_simdgroup]],
    uint sg [[simdgroup_index_in_threadgroup]], uint ns [[simdgroups_per_threadgroup]]
) {
    if (gid >= p.batch_tokens) return;
    uint d = p.d_model; uint bi = gid / p.seq_len; uint si = gid % p.seq_len;
    uint kv_dim = p.head_dim * p.n_kv_heads;
    device const float* xr = x + gid * d;
    // RMSNorm
    float lss = 0.0f;
    for (uint c = tid; c < d; c += tpg) { float v = xr[c]; lss += v*v; }
    float ss = simd_sum(lss);
    threadgroup float sv[8]; if (sl == 0) sv[sg] = ss;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    if (p.rank == 0) {
        ...
        for (uint i = tid; i < kv_dim; i += tpg) {
            float v=0; for (uint k=0;k<d;k++) v+=nx[k]*W_v[k*kv_dim+i];
            uint h=i/p.head_dim, e=i%p.head_dim;
            V_out[(bi*p.n_kv_heads+h)*p.seq_len*p.head_dim+si*p.head_dim+e]=v;
        }
    } else {
        uint r=p.rank; threadgroup float tl[64];
        for (uint j=tid;j<r;j+=tpg){float v=0;for(uint k=0;k<d;k++)v+=nx[k]*W_q[k*r+j];tl[j]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint i=tid;i<d;i+=tpg){float v=0;for(uint j=0;j<r;j++)v+=tl[j]*W_q_v[j*d+i];qb[i]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint j=tid;j<r;j+=tpg){float v=0;for(uint k=0;k<d;k++)v+=nx[k]*W_k[k*r+j];tl[j]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint i=tid;i<kv_dim;i+=tpg){float v=0;for(uint j=0;j<r;j++)v+=tl[j]*W_k_v[j*kv_dim+i];kb[i]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint j=tid;j<r;j+=tpg){float v=0;for(uint k=0;k<d;k++)v+=nx[k]*W_v[k*r+j];tl[j]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for (uint i=tid;i<kv_dim;i+=tpg){float v=0;for(uint j=0;j<r;j++)v+=tl[j]*W_v_v[j*kv_dim+i];
            uint h=i/p.head_dim,e=i%p.head_dim;
            V_out[(bi*p.n_kv_heads+h)*p.seq_len*p.head_dim+si*p.head_dim+e]=v;}
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    // RoPE + write Q,K to transposed layout
    uint pos=si+p.kv_offset;
    for (uint i=tid;i<d/2;i+=tpg) {
        uint fi=i*2; uint h=fi/p.head_dim; uint pi=(fi%p.head_dim)/2;
        float freq=1.0f/pow(p.rope_theta,2.0f*float(pi)/float(p.head_dim));
        float ang=float(pos)*freq; float ca=cos(ang),sa=sin(ang);
        float x0=qb[fi],x1=qb[fi+1]; qb[fi]=x0*ca-x1*sa; qb[fi+1]=x0*sa+x1*ca;
    }
    for (uint i=tid;i<kv_dim/2;i+=tpg) {
        uint fi=i*2; uint h=fi/p.head_dim; uint pi=(fi%p.head_dim)/2;
        float freq=1.0f/pow(p.rope_theta,2.0f*float(pi)/float(p.head_dim));
        float ang=float(pos)*freq; float ca=cos(ang),sa=sin(ang);
        float x0=kb[fi],x1=kb[fi+1]; kb[fi]=x0*ca-x1*sa; kb[fi+1]=x0*sa+x1*ca;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    for (uint i=tid;i<d;i+=tpg) {
        uint h=i/p.head_dim,e=i%p.head_dim;
        Q_out[(bi*p.n_heads+h)*p.seq_len*p.head_dim+si*p.head_dim+e]=qb[i];
    }
    for (uint i=tid;i<kv_dim;i+=tpg) {
        uint h=i/p.head_dim,e=i%p.head_dim;
        K_out[(bi*p.n_kv_heads+h)*p.seq_len*p.head_dim+si*p.head_dim+e]=kb[i];
    }
}
"#;

/// Fused post-attention + FFN: gather + W_o + residual + norm + SwiGLU + residual.
/// Each threadgroup handles one token. d_model ≤ 256, d_ff ≤ 1024.
pub const FUSED_POST_ATTN_FFN: &str = r#"
#include <metal_stdlib>
using namespace metal;
struct PostAttnFFNParams {
    uint batch_tokens; uint d_model; uint d_ff;
    uint n_heads; uint head_dim; uint seq_len; uint rank; float eps;
};
kernel void fused_post_attn_ffn(
    device const float* x_res [[buffer(0)]], device const float* attn_out [[buffer(1)]],
    device const float* norm_w [[buffer(2)]], device const float* W_o [[buffer(3)]],
    device const float* W_o_v [[buffer(4)]], device const float* W1 [[buffer(5)]],
    device const float* W2 [[buffer(6)]], device const float* W3 [[buffer(7)]],
    device float* output [[buffer(8)]], constant PostAttnFFNParams& p [[buffer(9)]],
    uint gid [[threadgroup_position_in_grid]], uint tid [[thread_index_in_threadgroup]],
    uint tpg [[threads_per_threadgroup]], uint sl [[thread_index_in_simdgroup]],
    uint sg [[simdgroup_index_in_threadgroup]], uint ns [[simdgroups_per_threadgroup]]
) {
    ...
    if (p.rank==0) {
        ...
        uint r=p.rank; threadgroup float tl[64];
        for(uint j=tid;j<r;j+=tpg){float v=0;for(uint k=0;k<d;k++)v+=ga[k]*W_o[k*r+j];tl[j]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        for(uint i=tid;i<d;i+=tpg){float v=0;for(uint j=0;j<r;j++)v+=tl[j]*W_o_v[j*d+i];wo[i]=v;}
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    // Residual + RMSNorm
    ...
    float ss=simd_sum(lss);
    threadgroup float sv2[8]; if(sl==0)sv2[sg]=ss;
    threadgroup_barrier(mem_flags::mem_threadgroup);
    float ts=0;for(uint i=0;i<ns;i++)ts+=sv2[i];
    float irms=rsqrt(ts/float(d)+p.eps);
    // Reuse ga for norm
    ...
    for(uint j=tid;j<ff;j+=tpg){
        ...
        float sg=1.0f/(1.0f+exp(-gv)); hid[j]=gv*sg*uv;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);
    // Down + residual
    device float* outr=output+gid*d;
    for(uint i=tid;i<d;i+=tpg){
        float dv=0;for(uint j=0;j<ff;j++)dv+=hid[j]*W2[j*d+i];
        outr[i]=h[i]+dv;
    }
}
"#;
...
/// 32 co-resident threadgroups × 64 threads with grid-level atomic barriers.
/// Tiled matmul (32×32, FP16 shared mem) for all projections.
/// Eliminates ~200 dispatch overhead per forward pass.
pub const PERSISTENT_LAYER: &str = r#"
#include <metal_stdlib>
using namespace metal;
#define PL_N 32
#define PL_T 32
...
struct PLParams {
    uint M; uint d; uint r; uint ff;
    uint nh; uint nkv; uint hd; uint seq;
    float eps; float theta; uint kv_off;
};

inline void pl_bar(device atomic_uint* c, uint gid, uint tid, uint gen) {
    threadgroup_barrier(mem_flags::mem_device);
    if (tid == 0) {
        uint tgt = (gen + 1) * PL_N;
        atomic_fetch_add_explicit(c, 1u, memory_order_relaxed);
        while (atomic_load_explicit(c, memory_order_relaxed) < tgt) {}
    }
    threadgroup_barrier(mem_flags::mem_device);
}
...
inline void pl_mm(
    ...
    uint M, uint N, uint K, uint gid, uint tid,
    threadgroup half* tA, threadgroup half* tB
) {
    uint lr=tid/8, lc=tid%8;
    uint ty=(M+PL_T-1)/PL_T, tx=(N+PL_T-1)/PL_T, tot=ty*tx;
    for (uint ti=gid; ti<tot; ti+=PL_N) {
        uint tr=(ti/tx)*PL_T, tc=(ti%tx)*PL_T;
        float acc[PL_TT][PL_TT]={{0.0f}};
        for (uint kb=0; kb<K; kb+=PL_T) {
            for (uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=tr+r,gc=kb+c;tA[r*PL_T+c]=(half)((gr<M&&gc<K)?A[gr*K+gc]:0.0f);}
            for (uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=kb+r,gc=tc+c;tB[r*PL_T+c]=(half)((gr<K&&gc<N)?B[gr*N+gc]:0.0f);}
            threadgroup_barrier(mem_flags::mem_threadgroup);
            for (uint k=0;k<PL_T;k++){
                half av[PL_TT],bv[PL_TT];
                for(uint i=0;i<PL_TT;i++)av[i]=tA[(lr*PL_TT+i)*PL_T+k];
                for(uint j=0;j<PL_TT;j++)bv[j]=tB[k*PL_T+lc*PL_TT+j];
                for(uint i=0;i<PL_TT;i++)for(uint j=0;j<PL_TT;j++)acc[i][j]+=(float)(av[i]*bv[j]);
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
        for(uint i=0;i<PL_TT;i++)for(uint j=0;j<PL_TT;j++){
            uint gr=tr+lr*PL_TT+i,gc=tc+lc*PL_TT+j;
            if(gr<M&&gc<N)C[gr*N+gc]=acc[i][j];}
    }
}

inline void pl_bmm_tb(
    device const float* A, device const float* B, device float* C,
    uint batch, uint M, uint N, uint K, uint gid, uint tid,
    threadgroup half* tA, threadgroup half* tB
) {
    uint lr=tid/8,lc=tid%8,ty=(M+PL_T-1)/PL_T,tx=(N+PL_T-1)/PL_T,tot=batch*ty*tx;
    for(uint ti=gid;ti<tot;ti+=PL_N){
        uint b=ti/(ty*tx),rem=ti%(ty*tx),tr=(rem/tx)*PL_T,tc=(rem%tx)*PL_T;
        device const float* Ab=A+b*M*K; device const float* Bb=B+b*N*K; device float* Cb=C+b*M*N;
        float acc[PL_TT][PL_TT]={{0.0f}};
        for(uint kb=0;kb<K;kb+=PL_T){
            for(uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=tr+r,gc=kb+c;tA[r*PL_T+c]=(half)((gr<M&&gc<K)?Ab[gr*K+gc]:0.0f);}
            for(uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=tc+r,gc=kb+c;tB[r*PL_T+c]=(half)((gr<N&&gc<K)?Bb[gr*K+gc]:0.0f);}
            threadgroup_barrier(mem_flags::mem_threadgroup);
            for(uint k=0;k<PL_T;k++){half av[PL_TT],bv[PL_TT];
                for(uint i=0;i<PL_TT;i++)av[i]=tA[(lr*PL_TT+i)*PL_T+k];
                for(uint j=0;j<PL_TT;j++)bv[j]=tB[(lc*PL_TT+j)*PL_T+k];
                for(uint i=0;i<PL_TT;i++)for(uint j=0;j<PL_TT;j++)acc[i][j]+=(float)(av[i]*bv[j]);}
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
        for(uint i=0;i<PL_TT;i++)for(uint j=0;j<PL_TT;j++){
            uint gr=tr+lr*PL_TT+i,gc=tc+lc*PL_TT+j;
            if(gr<M&&gc<N)Cb[gr*N+gc]=acc[i][j];}
    }
}
...
inline void pl_bmm(
    ...
    for(uint ti=gid;ti<tot;ti+=PL_N){
        uint b=ti/(ty*tx),rem=ti%(ty*tx),tr=(rem/tx)*PL_T,tc=(rem%tx)*PL_T;
        device const float* Ab=A+b*M*K; device const float* Bb=B+b*K*N; device float* Cb=C+b*M*N;
        float acc[PL_TT][PL_TT]={{0.0f}};
        for(uint kb=0;kb<K;kb+=PL_T){
            for(uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=tr+r,gc=kb+c;tA[r*PL_T+c]=(half)((gr<M&&gc<K)?Ab[gr*K+gc]:0.0f);}
            for(uint i=0;i<16;i++){uint f=tid*16+i,r=f/PL_T,c=f%PL_T;
                uint gr=kb+r,gc=tc+c;tB[r*PL_T+c]=(half)((gr<K&&gc<N)?Bb[gr*N+gc]:0.0f);}
            threadgroup_barrier(mem_flags::mem_threadgroup);
            for(uint k=0;k<PL_T;k++){half av[PL_TT],bv[PL_TT];
                for(uint i=0;i<PL_TT;i++)av[i]=tA[(lr*PL_TT+i)*PL_T+k];
                for(uint j=0;j<PL_TT;j++)bv[j]=tB[k*PL_T+lc*PL_TT+j];
...
inline void pl_norm(device const float* x, device const float* w, device float* out,
    uint M, uint d, float eps, uint gid, uint tid, threadgroup float* red) {
    for(uint row=gid;row<M;row+=PL_N){
        device const float* xr=x+row*d; device float* or_=out+row*d;
        float ss=0;for(uint c=tid;c<d;c+=64)ss+=xr[c]*xr[c];
        ss=simd_sum(ss);uint sg=tid/32,sl=tid%32;
        if(sl==0)red[sg]=ss;threadgroup_barrier(mem_flags::mem_threadgroup);
        float ir=rsqrt((red[0]+red[1])/float(d)+eps);
        for(uint c=tid;c<d;c+=64)or_[c]=xr[c]*ir*w[c];
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
}

inline void pl_add(device const float* a, device const float* b, device float* c, uint n, uint gid, uint tid) {
    for(uint i=gid*64+tid;i<n;i+=PL_N*64)c[i]=a[i]+b[i];
}
inline void pl_silu_gate(device const float* g, device const float* u, device float* o, uint n, uint gid, uint tid) {
    for(uint i=gid*64+tid;i<n;i+=PL_N*64){float gv=g[i];o[i]=gv/(1.0f+exp(-gv))*u[i];}
}
inline void pl_tr_bsh(device const float* s, device float* dst, uint batch, uint seq, uint heads, uint hd, uint gid, uint tid) {
    uint dm=heads*hd,tot=batch*seq*dm;
    for(uint i=gid*64+tid;i<tot;i+=PL_N*64){uint tk=i/dm,w=i%dm,b=tk/seq,si=tk%seq,h=w/hd,e=w%hd;
        dst[(b*heads+h)*seq*hd+si*hd+e]=s[i];}
}
inline void pl_tr_bhs(device const float* s, device float* dst, uint batch, uint seq, uint heads, uint hd, uint gid, uint tid) {
    uint tot=batch*heads*seq*hd;
    for(uint i=gid*64+tid;i<tot;i+=PL_N*64){uint bh=i/(seq*hd),rm=i%(seq*hd),si=rm/hd,e=rm%hd;
        uint b=bh/heads,h=bh%heads;dst[(b*seq+si)*(heads*hd)+h*hd+e]=s[i];}
}
inline void pl_rope(device float* buf, uint bh, uint seq, uint hd, float theta, uint off, uint gid, uint tid) {
    uint np=bh*seq*(hd/2);
    for(uint i=gid*64+tid;i<np;i+=PL_N*64){uint pi=i%(hd/2),rest=i/(hd/2),si=rest%seq;
        float freq=1.0f/pow(theta,2.0f*float(pi)/float(hd)),ang=float(si+off)*freq;
        float ca=cos(ang),sa=sin(ang);uint base=(rest/seq)*seq*hd+si*hd+pi*2;
        float x0=buf[base],x1=buf[base+1];buf[base]=x0*ca-x1*sa;buf[base+1]=x0*sa+x1*ca;}
}
inline void pl_softmax(device float* sc, uint bh, uint sq, uint sk, float scale, uint off, uint gid, uint tid, threadgroup float* red) {
    for(uint row=gid;row<bh*sq;row+=PL_N){device float* sr=sc+row*sk;uint qp=(row%sq)+off;
        for(uint c=tid;c<sk;c+=64){float v=sr[c]*scale;if(c>qp)v=-1e9f;sr[c]=v;}
        threadgroup_barrier(mem_flags::mem_threadgroup);
        float mx=-1e9f;for(uint c=tid;c<sk;c+=64)mx=max(mx,sr[c]);mx=simd_max(mx);
        uint sg=tid/32,sl=tid%32;if(sl==0)red[sg]=mx;threadgroup_barrier(mem_flags::mem_threadgroup);
        float rmax=max(red[0],red[1]);float sm=0;
        for(uint c=tid;c<sk;c+=64){float e=exp(sr[c]-rmax);sr[c]=e;sm+=e;}
        sm=simd_sum(sm);if(sl==0)red[sg]=sm;threadgroup_barrier(mem_flags::mem_threadgroup);
        float inv=1.0f/(red[0]+red[1]+1e-10f);for(uint c=tid;c<sk;c+=64)sr[c]*=inv;
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }
}

kernel void persistent_layer(
    device const float* ln1_w [[buffer(0)]], device const float* Wq [[buffer(1)]],
    device const float* Wk [[buffer(2)]], device const float* Wv [[buffer(3)]],
    device const float* Wqv [[buffer(4)]], device const float* Wkv [[buffer(5)]],
    device const float* Wvv [[buffer(6)]], device const float* Wo [[buffer(7)]],
    device const float* Wov [[buffer(8)]], device const float* ln2_w [[buffer(9)]],
    device const float* W1 [[buffer(10)]], device const float* W2 [[buffer(11)]],
    device const float* W3 [[buffer(12)]], device float* x_in [[buffer(13)]],
    device float* scratch [[buffer(14)]], device atomic_uint* bar [[buffer(15)]],
    constant PLParams& p [[buffer(16)]],
    device const float* W1v [[buffer(17)]], device const float* W2v [[buffer(18)]],
    device const float* W3v [[buffer(19)]],
    uint gid [[threadgroup_position_in_grid]], uint tid [[thread_index_in_threadgroup]]
) {
    threadgroup half tA[1024];
    threadgroup half tB[1024];
    threadgroup float red[2];

    uint M=p.M,d=p.d,r=p.r,ff=p.ff,nh=p.nh,nkv=p.nkv,hd=p.hd,seq=p.seq;
    uint batch=M/seq,bh=batch*nh,bkv=batch*nkv,kvd=nkv*hd;
    uint gen=0;

    // Scratch offsets (floats)
    uint o0=0,o1=o0+M*d,o2=o1+M*(r>0?r:d),o3=o2+M*(r>0?r:kvd),o4=o3+M*(r>0?r:kvd);
    uint o5=o4+bh*seq*hd,o6=o5+bkv*seq*hd,o7=o6+bkv*seq*hd,o8=o7+bh*seq*seq;
    uint o9=o8+bh*seq*hd,oA=o9+M*d,oB=oA+M*(r>0?r:d),oC=oB+M*d,oD=oC+M*d;
    uint oE=oD+M*(r>0?r:ff),oF=oE+M*(r>0?r:ff),oG=oF+M*ff,oH=oG+M*ff,oI=oH+M*ff;
    uint oJ=oI+M*(r>0?r:d);

    device float* bnm=scratch+o0,*btq=scratch+o1,*btk=scratch+o2,*btv=scratch+o3;
    device float* bQ=scratch+o4,*bK=scratch+o5,*bV=scratch+o6,*bsc=scratch+o7;
    device float* bao=scratch+o8,*baf=scratch+o9,*bwt=scratch+oA,*bh_=scratch+oB;
    device float* bn2=scratch+oC,*bgt=scratch+oD,*but_=scratch+oE,*bga=scratch+oF;
    device float* bup=scratch+oG,*bhi=scratch+oH,*bdt=scratch+oI,*bdn=scratch+oJ;

    // P1: RMSNorm
    pl_norm(x_in, ln1_w, bnm, M, d, p.eps, gid, tid, red); pl_bar(bar,gid,tid,gen++);

    // P2-3: QKV
    if (r>0) {
        pl_mm(bnm,Wq,btq,M,r,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bnm,Wk,btk,M,r,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bnm,Wv,btv,M,r,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(btq,Wqv,baf,M,d,r,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(btk,Wkv,btk,M,kvd,r,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(btv,Wvv,btv,M,kvd,r,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    } else {
        pl_mm(bnm,Wq,baf,M,d,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bnm,Wk,btk,M,kvd,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bnm,Wv,btv,M,kvd,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    }

    // P4: Transpose + RoPE
    pl_tr_bsh(baf,bQ,batch,seq,nh,hd,gid,tid);
    pl_tr_bsh(btk,bK,batch,seq,nkv,hd,gid,tid);
    pl_tr_bsh(btv,bV,batch,seq,nkv,hd,gid,tid);
    pl_bar(bar,gid,tid,gen++);
    pl_rope(bQ,bh,seq,hd,p.theta,p.kv_off,gid,tid);
    pl_rope(bK,bkv,seq,hd,p.theta,p.kv_off,gid,tid);
    pl_bar(bar,gid,tid,gen++);

    // P5: Q@K^T
    pl_bmm_tb(bQ,bK,bsc,bh,seq,seq,hd,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);

    // P6: Softmax
    pl_softmax(bsc,bh,seq,seq,1.0f/sqrt(float(hd)),p.kv_off,gid,tid,red); pl_bar(bar,gid,tid,gen++);

    // P7: Attn@V
    pl_bmm(bsc,bV,bao,bh,seq,hd,seq,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);

    // P8: Transpose back + W_o
    pl_tr_bhs(bao,baf,batch,seq,nh,hd,gid,tid); pl_bar(bar,gid,tid,gen++);
    if (r>0) {
        pl_mm(baf,Wo,bwt,M,r,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bwt,Wov,baf,M,d,r,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    } else {
        pl_mm(baf,Wo,bwt,M,d,d,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    }

    // P9: Residual + RMSNorm
    device float* ap=(r>0)?baf:bwt;
    pl_add(x_in,ap,bh_,M*d,gid,tid); pl_bar(bar,gid,tid,gen++);
    pl_norm(bh_,ln2_w,bn2,M,d,p.eps,gid,tid,red); pl_bar(bar,gid,tid,gen++);

    // P10-11: FFN
    if (r>0) {
        pl_mm(bn2,W1,bgt,M,r,d,gid,tid,tA,tB); pl_mm(bn2,W3,but_,M,r,d,gid,tid,tA,tB);
        pl_bar(bar,gid,tid,gen++);
        pl_mm(bgt,W1v,bga,M,ff,r,gid,tid,tA,tB); pl_mm(but_,W3v,bup,M,ff,r,gid,tid,tA,tB);
        pl_bar(bar,gid,tid,gen++);
    } else {
        pl_mm(bn2,W1,bga,M,ff,d,gid,tid,tA,tB); pl_mm(bn2,W3,bup,M,ff,d,gid,tid,tA,tB);
        pl_bar(bar,gid,tid,gen++);
    }
    pl_silu_gate(bga,bup,bhi,M*ff,gid,tid); pl_bar(bar,gid,tid,gen++);

    // P12: Down
    if (r>0) {
        pl_mm(bhi,W2,bdt,M,r,ff,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
        pl_mm(bdt,W2v,bdn,M,d,r,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    } else {
        pl_mm(bhi,W2,bdn,M,d,ff,gid,tid,tA,tB); pl_bar(bar,gid,tid,gen++);
    }

    // P13: Final residual
    pl_add(bh_,bdn,x_in,M*d,gid,tid); pl_bar(bar,gid,tid,gen++);
}
"#;
